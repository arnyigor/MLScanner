...
### DI модули (`di/`)
- `AppModule` — инициализация приложения
...
**Логика HybridEngine:**
1. Первым запускается Tesseract (лучше справляется с русским)
2. Если Tesseract вернул непустой результат → сразу возвращаем результат
3. ML Kit используется как fallback при пустом результате Tesseract
4. При наличии результатов от обоих движков выбирается результат с большим количеством слов
...
- `DocumentDetector` — автоматическое нахождение границ документа (OpenCV + Canny edge detection)

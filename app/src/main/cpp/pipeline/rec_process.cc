...
cv::Mat CrnnResizeImg(cv::Mat img, float wh_ratio, int rec_image_height) {
  int imgH = rec_image_height;
  int imgW = int(rec_image_height * wh_ratio);
...
RecPredictor::RecPredictor(const std::string &modelDir, const int cpuThreadNum,
                           ...
                           const int recImageHeight)
    : recImageHeight_(recImageHeight > 0 ? recImageHeight : 32) {
...
void RecPredictor::Preprocess(const cv::Mat &srcimg) {
  ...
  cv::Mat resize_img = CrnnResizeImg(srcimg, wh_ratio, recImageHeight_);

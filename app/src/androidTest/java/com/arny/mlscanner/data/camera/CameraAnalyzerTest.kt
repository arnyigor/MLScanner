...
class CameraAnalyzerTest {
    ...
    @Mock
    private lateinit var mockAdvancedRecognizeTextUseCase: AdvancedRecognizeTextUseCase
    ...
    fun setUp() {
        ...
        cameraAnalyzer = CameraAnalyzer(
            ...
            advancedRecognizeTextUseCase = mockAdvancedRecognizeTextUseCase
        )
    }
    ...
    fun `should analyze image proxy and trigger OCR`() = runTest {
        ...
        whenever(mockAdvancedRecognizeTextUseCase.execute(any(), any())).thenReturn(ocrResult)
        ...
        verify(mockAdvancedRecognizeTextUseCase, times(1)).execute(any(), any())
    ...
    fun `should handle OCR processing errors`() = runTest {
        ...
        whenever(mockImageProxy.height).thenReturn(480)
        whenever(mockImageProxy.planes).thenReturn(arrayOf())
        whenever(mockAdvancedRecognizeTextUseCase.execute(any(), any())).thenThrow(RuntimeException("OCR failed"))
    ...
    fun `should handle different image formats`() = runTest {
        ...
        for (format in formats) {
            ...
            whenever(mockImageProxy.planes).thenReturn(arrayOf())
            whenever(mockAdvancedRecognizeTextUseCase.execute(any(), any())).thenReturn(createTestOcrResult())
    ...
    private fun createTestOcrResult(): OcrModels.OcrResult {
        return OcrResult(
            textBoxes = listOf(
                OcrModels.TextBox(
                    text = "Test text from camera",
                    confidence = 0.85f,
                    boundingBox = OcrModels.BoundingBox(100f, 100f, 300f, 150f)
                )
            ),
